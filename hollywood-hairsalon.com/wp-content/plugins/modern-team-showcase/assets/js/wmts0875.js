...
jQuery( function( $ ){
		...
		$showcase.magnificPopup( {
				...
				callbacks: {
						elementParse: function( item ){
								...
								$( 'html' ).addClass( 'wmts_lightbox_active' ); // iPhone lightbox scroll fix
						...
						change: function( item ){
								...
								$image.css( 'visibility', 'hidden' ).attr( 'src', image_src ).centerImage( 'outside', function( ){ $image.css( 'visibility', 'visible' ); } );
							},
						close: function( item ){
	...
	wmts.showcase.resize = function( $showcase ){
		...
		$showcase.addClass( 'wmts_isotope_arranging' );
		...
		var  device_settings = wmts.showcase.get_settings( $showcase ).settings,
				...
				columns = device_settings.columns || 3,
		...
		$mc_children.css( {'margin-bottom': device_settings.cellSpace, 'fontSize': device_settings.fontSize +'px' } );
		// height calc
		...
		// center image
		var $images = $( '.wmts_image_centering>img', $showcase ),
				total_images = $images.length;
		$images.each( function( ){
			var 	$this = $( this ),
					$parent = $this.parent( ),
					rounded = $parent.hasClass( 'wmts_rounded' );
					type = $parent.hasClass( 'wmts_inside' ) && ! rounded ? 'inside' : false;

			$this.addClass( 'wmts_isotope_arranging' );

			// parent needs to be a square before rounding
			if( rounded ){
				$parent.width( '' );
				$parent.height( '' );
				var parent_width = $parent.width(),
						parent_height = $parent.height(),
						diff = parent_width - parent_height;

				if( diff > 0 ){
					$parent.width( parent_height );
				}else{
					$parent.height( parent_width );
				}
			}

			$this.centerImage( type, function( ){

				// case of 'outside' centering
				if( ! type ){
					var 	parent_width = $parent.width( ),
							parent_height = $parent.height( ),
							parent_prop = parent_width / parent_height,
							image_width = $this[ 0 ].naturalWidth,
							image_height = $this[ 0 ].naturalHeight,
							image_prop = image_width / image_height;
					if( image_prop > parent_prop ){ // image is proportionately wider than container
						$this.css( { 'width': 'auto', 'height': parent_height + 'px' } );
						$this.css( { 'left': ( parent_width - $this.width() ) / 2 +'px', 'top': '0' } );
					}else{ // image is proportionately taller than container
						$this.css( { 'width': parent_width + 'px', 'height': 'auto' } );
						$this.css( { 'left': '0', 'top': ( parent_height - $this.height() ) / 2 +'px' } );
					}
				}

				// apply grayscale
				if( ( $parent.hasClass( 'wmts_grayscale' ) || $parent.hasClass( 'wmts_grayscale_' ) ) && $this.attr( 'src' ) !== 'about:blank' ){
					var $canvas = $this.siblings( 'canvas' );
					if( $canvas.length ){
						//just copy image centering style to canvas
						var style = $this.attr( 'style' );
						$canvas.attr( 'style', style );
					}else{
						// create canvas if it is not there
						var 	$clone = $this.clone( ),
								elm_id = $this.attr( 'data-wph-elm' );
						$clone.insertAfter( $this );
						$parent.removeClass( 'wmts_grayscale' ).addClass( 'wmts_grayscale_' );
						$this.next( ).pixastic( 'desaturate' ).attr( 'data-wph-elm', elm_id );
					}
				}

				var $siblings = $this.siblings( 'canvas' );
				if( $siblings.length ){
					var $all = $this.add( $siblings );
					$all.removeClass( 'wmts_isotope_arranging' );
				}else{
					$this.removeClass( 'wmts_isotope_arranging' );
				}

				total_images--;
				if( ! total_images ) $showcase.addClass( 'wmts_loaded' ); // just loaded the last image
	...
	wmts.showcase.layoutCompleteCallback= function( o ){
		if( typeof o.element !== 'undefined' ) $( o.element ).closest( '.wmts_container' ).removeClass( 'wmts_isotope_arranging' );
		else{
			if( typeof o[ 0 ] === 'undefined' ) return;
			$( o[ 0 ].element ).closest( '.wmts_container' ).removeClass( 'wmts_isotope_arranging' );
		}
	}
	...
	wmts.showcase.get_settings = function( $showcase ){
		...
		if( devices_settings ){
			$.each( devices_settings, function( key, val ){
				if( val[ 'breakpoint' ] < screen_width  ) {
					device = key;
					val.cellSpace = parseFloat( val.cellSpace );
					return false;
				}
			} )
		}
	...
	$( window ).on( 'resize', function( e ){
		...
		wmts.resizeTimer = setTimeout( function( ){
				...
				if( curr_window_width !== window_width ){
					...
					$( '.mfp-content .wmts_image_centering>img' ).centerImage( );
				}
		}, 250 );
	});
	...
	function wmts_inital_process( $showcase ){
		...
		// ensures neither completes before the other even starts
 ...
 */
!function(t){t.fn.centerImage=function(i,e){e=e||function(){};var n=this,h=t(this).length;i="inside"==i;var o=function(e){var n=t(e),h=n.parent();h.css({overflow:"hidden",position:"absolute"==h.css("position")?"absolute":"relative"}),n.css({position:"static",width:"auto",height:"auto","max-width":"100%","max-height":"100%"});var o={w:h.width(),h:h.height(),r:h.width()/h.height()},e={w:n.width(),h:n.height(),r:n.width()/n.height()};n.css({"max-width":"none","max-height":"none",width:Math.round(o.r>e.r^i?"100%":o.h/e.h*e.w),height:Math.round(o.r<e.r^i?"100%":o.w/e.w*e.h)});var o={w:h.width(),h:h.height()},e={w:n.width(),h:n.height()};n.css({position:"absolute",left:Math.round((o.w-e.w)/2),top:Math.round((o.h-e.h)/3)}),r(e)},r=function(t){h--,e.apply(n,[t,h])};return n.each(function(){this.complete||"complete"===this.readyState?!function(t){setTimeout(function(){o(t)},1)}(this):!function(i){t(i).one("load",function(){setTimeout(function(){o(i)},1)}).one("error",function(){r(i)}).end(),(navigator.userAgent.indexOf("Trident/5")>=0||navigator.userAgent.indexOf("Trident/6"))&&(i.src=i.src)}(this)})},t.fn.imageCenterResize=function(i){return t(this).centerImage("inside",i)},t.fn.imageCropFill=function(i){return t(this).centerImage("outside",i)}}(jQuery);
}
